...
if(!empty($_REQUEST['time'])) {
	...
	$last_modified = time();
	$title = _('Soundbox: más votadas en') . ' ' . txt_time_diff($from);
	...
	$title = _('Soundbox: favoritas de') . ' ' . $user_login;
	...
	$user_login = $db->get_var("select user_login from users where user_id=$user_id");
	$title = _('Soundbox: votadas por') . ' ' . $user_login;
	$globals['redirect_feedburner'] = false;
	...
	$user_login = $db->get_var("select user_login from users where user_id=$user_id");
	$title = _('Soundbox: amigos de') . ' ' . $user_login;
	$globals['redirect_feedburner'] = false;
	...
	$user_login = $db->get_var("select user_login from users where user_id=$user_id");
	$title = _('Soundbox: noticias de') . ' ' . $user_login;
	$globals['redirect_feedburner'] = false;
} else {
	/////
	// All the others
	...
	switch ($status) {
		case 'published':
			...
			$link_date = 'date';
			$title = _('Soundbox: publicadas');
			break;
		case 'queued':
			$title = _('Soundbox: en cola');
			$order_field = 'link_date';
			$link_date = "date";
			$home = "/shakeit.php";
	...
	if($_REQUEST['q']) {
		...
		$title = _('Soundbox') . ": " . htmlspecialchars(strip_tags($_REQUEST['q']));
	}
...
if ($links) {
	foreach($links as $link_id) {
		...
		// soundbox own namespace
		echo "		<soundbox:user>$link->username</soundbox:user>\n";
		echo "		<soundbox:votes>".intval($link->votes+$link->anonymous)."</soundbox:votes>\n";
		echo "		<soundbox:negatives>$link->negatives</soundbox:negatives>\n";
		echo "		<soundbox:karma>".intval($link->karma)."</soundbox:karma>\n";
		echo "		<soundbox:comments>$link->comments</soundbox:comments>\n";
		echo "		<soundbox:url>".htmlspecialchars($link->url)."</soundbox:url>\n";
...
function do_header($title) {
	...
	echo '	xmlns:soundbox="http://soundbox.es/faq-es.php"'."\n";
	echo ' >'. "\n";
	echo '<channel>'."\n";
	...
	//echo'	<generator>http://blog.soundbox.net/</generator>'."\n";
...
function check_redirect_to_feedburner($status) {
	...
	switch ($status) {
		case 'published':
			//header("Location: http://feeds.feedburner.com/soundbox/published");
			header("Location: http://feedproxy.google.com/soundboxPublicadas");
			exit();
			break;
		case 'queued':
			//header("Location: http://feeds.feedburner.com/soundbox/queued");
			header("Location: http://feedproxy.google.com/soundboxEnCola");
			exit();
			break;
		case 'all':
			//header("Location: http://feeds.feedburner.com/soundbox/all");
			header("Location: http://feedproxy.google.com/soundboxEnCola");
			exit();
			break;
	}
