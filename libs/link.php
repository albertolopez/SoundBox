...
class Link {
	...
	function print_summary($type='full', $karma_best_comment = 0, $show_tags = true) {
		...
		if (($src = $this->has_thumb())) {
			echo "<img src='$src' width='$this->thumb_x' height='$this->thumb_y' alt='' class='thumbnailnoticia'/>";
		}
		...
		if($type=='full' || $type=='preview') {
			echo '<p>';
			echo '<span class="website"><a href="search.php?q='.getDomain($this->url).'&amp;p=site&amp;s=published">'.str_ireplace('www.', '',parse_url($this->url,PHP_URL_HOST))."</a></span> &mdash; ";
	...
	function print_shake_box() {
		...
		echo '<a id="a-votes-'.$this->id.'" href="'.$this->get_relative_permalink().'">'.($this->votes+$this->anonymous).'</a>'._('zumbidos').'</div>';
		...
		if (! $globals['bot']) {
			...
			if ($this->votes_enabled == false) {
				...
				echo '<a href="javascript:menealo('."$current_user->user_id,$this->id".')" id="a-shake-'.$this->id.'">'._('zúmbalo').'</a>';
			} else {
...
//		echo '<input type="hidden" name="return" value="" disabled />';
	...
	function get_thumb($debug = false) {
		...
		$this->thumb = '';
		if(!$img){
			$this->image_parser2 = new HtmlImages("http://www.artviper.net/screenshots/screener.php?url=".$this->url."&amp;w=".($globals['thumb_size'])."&amp;sdx=1280&amp;userID=a925e187d7f02372", $site);
			$this->image_parser2->debug = $debug;
			$this->image_parser2->referer = $this->get_permalink();
			echo "<!-- Meneame, before image_parser -->\n";
			$img = $this->image_parser2->get();
			echo "<!-- Meneame, after image_parser -->\n";
			$this->thumb_status = 'checked';
			$this->thumb = '';
		}
